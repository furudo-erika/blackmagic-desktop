...
### Fixed
- **Sidebar rows left-aligned.** The expandable rows (Agents, Chat
  History, Knowledge) were using `px-2 py-1.5 gap-2 w-3.5 h-3.5` while
  the flat NavRow used `px-3 py-2 gap-2.5 w-4 h-4`. The 4-px padding
  gap + 2-px icon-size delta gave the left rail a faintly serrated
  left edge. Unified every row to the NavRow system; every icon now
...
- **Every starter prompt rewritten to match its agent's real doctrine.**
 ...
 - X Account starters match the playbook cadence the agent
...
### Fallbacks
- Domain-keyed fallback slots for `vercel.com` so
...
- **Triggers page now shows last-run status per row.** Previously the "view
 ...
 and invalidated immediately after a Fire now. Makes the 9 cron
...
- **X Account Agent (#15).** Mirrors a proven in-house Twitter
...
- **Brand Monitor Agent (#14)** — dedicated agent for Reddit + X
 brand-keyword monitoring, modelled on what top B2B SaaS teams run in-house.
...
- **Brand monitor returned unrelated Reddit posts as "mentions".**
 ...
 from Reddit's firehose. Example bug report: searching for `<brand>`
 surfaced an AutoModerator welcome post in r/Morocco that mentioned
 neither brand anywhere. The skills also had no
...
- **`requires:` frontmatter on every new default skill** — brand-
 ...
 files), api-endpoint-test (needs a REST test CLI),
...
- **7 new skills + 3 new BYOK integrations, all generalized from the
 in-house corpus.** None of the skills hardcode a vendor, a
 ...
 **Tier 1 — zero-integration-dependency skills:**
 ...
 `the CLI` (free npm package) against any REST backend. Discovers
 routes from Next/Express/FastAPI code or an OpenAPI spec; covers
 auth / validation / method / 404 / happy-path. No account
 needed.
...
- **Five Apify-driven Skills, vendor-neutral, ship in every vault.**
 Generalized from the in-house pipelines so any user with their
...
- **End-to-end LinkedIn outreach loop** — new universal skill
 ...
 `signals/linkedin/<date>-loop.md`. Replaces the earlier
