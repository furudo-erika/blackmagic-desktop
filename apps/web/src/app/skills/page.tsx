...
export default function SkillsPage() {
  ...
  useEffect(() => {
    ...
    if (!selectedSlug && first) {
      router.replace(`/skills?skill=${encodeURIComponent(skillSlug(first))}`);
    }
      ...
      <header className="shrink-0 border-b border-line dark:border-[#2A241D] px-5 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-muted dark:text-[#8C837C] truncate">
            Capabilities your agents invoke during a conversation. Pick one to inspect what it does — agents pull these in automatically when the task fits.
          </p>
        </div>
